...
const StepThree = ({ formData, handleChange, handleFileChange, handleBack, setStep }) => {
  ...
  const isFormValid = () => {
    return formData.dateOfBirth && formData.profileImage && !errors.dateOfBirth;
  };
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        ...
        <div className={`w-full h-44 mt-1 border-2 ${!formData.profileImage ? "border-blue-400" : "border-gray-300"} border-dashed rounded-md flex items-center justify-center relative`}>
          {imagePreview ? (
            <img
              src={imagePreview}
              alt="Preview"
      ...
      <div className="flex justify-between mt-6">
        ...
        <button
          ...
          className={`ml-3 w-[100%] py-3 rounded-md after:content-['>'] after:ml-3 
            ${isFormValid() ? 'bg-[#202124] text-[#FFFFFF]' : 'bg-[#D6D8DB] text-[#A9ACAF] opacity-50 cursor-not-allowed'}`}
        >
          Submit 3/3
        </button>
      </div>
